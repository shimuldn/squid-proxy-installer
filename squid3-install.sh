...
# Github: https://github.com/shimuldn/squid-proxy-installer/
...
if cat /etc/os-release | grep PRETTY_NAME | grep "Ubuntu 22.04"; then
    ...
    /usr/bin/wget --no-check-certificate -O /etc/squid/squid.conf https://raw.githubusercontent.com/shimuldn/squid-proxy-installer/master/conf/ubuntu-2204.conf
    ...
    /bin/rm -f /etc/squid/squid.conf
    /usr/bin/touch /etc/squid/blacklist.acl
    /usr/bin/wget --no-check-certificate -O /etc/squid/squid.conf https://raw.githubusercontent.com/shimuldn/squid-proxy-installer/master/conf/squid-centos7.conf
    systemctl enable squid
    systemctl restart squid
